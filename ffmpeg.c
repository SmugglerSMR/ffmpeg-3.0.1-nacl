...
static PP_Resource urlLoader;
static PP_Resource urlRequestInfo;
...
// ---------------------------------------------------------
static int check_fvdmedia(const char* url) {

    g_pInstance = (PPB_Instance*) PSGetInterface(PPB_INSTANCE_INTERFACE);
    ppb_urlloader_interface = (PPB_URLLoader*) PSGetInterface(PPB_URLLOADER_INTERFACE);
    ppb_urlrequestinfo_interface = (PPB_URLRequestInfo*) PSGetInterface(PPB_URLREQUESTINFO_INTERFACE); 
    ppb_var_interface = (PPB_Var*) PSGetInterface(PPB_VAR_INTERFACE); 

    if (ppb_var_interface == NULL) {
        printf("ppb_var_interface - create failure\n");
        return -10;
    }

    urlLoader = ppb_urlloader_interface->Create( PSGetInstanceId() );
    if (urlLoader == 0) {
        printf("urlLoader - create failure\n");
        return -10;
    }

    urlRequestInfo = ppb_urlrequestinfo_interface->Create( PSGetInstanceId() );
    if (urlRequestInfo == 0) {
        printf("urlRequestInfo - create failure\n");
        return -10;
    }

    ppb_urlrequestinfo_interface->SetProperty(urlRequestInfo, PP_URLREQUESTPROPERTY_URL, CStrToVar(url));
    ppb_urlrequestinfo_interface->SetProperty(urlRequestInfo, PP_URLREQUESTPROPERTY_METHOD, CStrToVar("GET"));
    ppb_urlrequestinfo_interface->SetProperty(urlRequestInfo, PP_URLREQUESTPROPERTY_RECORDDOWNLOADPROGRESS, PP_MakeBool(PP_TRUE));

    struct PP_CompletionCallback cb_open = PP_BlockUntilComplete();
    int val = ppb_urlloader_interface->Open(urlLoader, urlRequestInfo, cb_open);  
    ...
    ppb_urlloader_interface->GetDownloadProgress(urlLoader, &cur, &total);
    ...
    bytes = ppb_urlloader_interface->ReadResponseBody(urlLoader, buffer, cnt, cb_open);

    if ( bytes < 100 ) {
        printf("ppb_urlloader_interface - read failure\n");
        return -10;
    }    
...
int ppapi_simple_main(int argc, char* argv[]) {
	...
	umount("/html5_persistent");
	ret = mount("", "/html5_persistent", "html5fs", 0, "type=PERSISTENT");
	if(ret) {
